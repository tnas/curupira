...
public class DigitoParaTest {
	...
	public void geracaoDeDigitoVerificadorParaBoleto() {
		...
		for (DigitoPara digitoPara : entradas.keySet()) {
			...
			assertEquals(esperado, digitoPara.comMultiplicadoresDeAte(2,9).complementarAoModulo().mod(11).calcula());
		}
	}
	...
	public void geracaoDeDigitoVerificadorParaBoletoCasosEspeciais() {
		...
		for (DigitoPara digitoPara : entradas.keySet()) {
			String esperado = entradas.get(digitoPara);
			assertEquals(esperado, digitoPara.comMultiplicadoresDeAte(2,9).complementarAoModulo().trocandoPorSeEncontrar("1", 0, 10, 11).mod(11).calcula());
		}
	}
	
	@Test
	public void geracaoDeDigitoMod11PraIntervaloPassado() {
		assertEquals("1", new DigitoPara("05009401448").comMultiplicadores(9,8,7,6,5,4,3,2).mod(11).calcula());
	}

	@Test
	public void geracaoDeDigitoParaRGDeSaoPaulo() {
		assertEquals("1", new DigitoPara("36422911").comMultiplicadoresDeAte(2,9).mod(11).calcula());
		assertEquals("X", new DigitoPara("42105900").comMultiplicadoresDeAte(2,9).trocandoPorSeEncontrar("X", 10).mod(11).calcula());
	}
